...
const EditListModal = ({
  ...
  function handlePlayCurrentSound(soundList, index) {
    soundList.current.forEach((ref, refIndex) => {
      if (refIndex !== index) {
        ...
        soundList.current[refIndex].current.currentTime = 0;
      }
    });
  ...
  function handleRemoveSoundFromLocalList(sound) {
    const tempList = localList.sounds.filter((localStorageSound, index) => {
    ...
    localStorage.setItem('nonUserList', JSON.stringify([localList]));
    ...
    <article className="bg-slate-800 p-4 rounded text-white sm:max-w-[600px] w-full">
      ...
      {localList.sounds.length < 1 ? (
        <div className="mb-4 w-full flex justify-center">
          點擊<span className="material-symbols-outlined">add</span>添加
        </div>
      ) : (
      ...
      {loading ? (
        <p className="mx-auto w-full h-[250px] grid place-content-center">
          <Spinner />
        </p>
      ) : (
          ...
          {sounds?.map((sound, index) => (
            <li
              ...
              className={`
                ...
                ${
                  localList.sounds.find(
                    (localSound) => localSound.name === sound.name
                  )
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Dialog.Panel className="sm:max-w-[600px] w-full">
            <EditListModal
          ...
          <Dialog.Panel>
            <div className="bg-slate-700 p-4 rounded text-white">
              <Dialog.Title className="mb-4">
                刪除列表: <span className="text-2xl">{localList?.name}</span>
              </Dialog.Title>
      ...
      <Transition appear show={isCreating}>
        ...
        <Dialog
          ...
          className={
            'fixed top-0 left-0 md:inset-0 flex items-center justify-center p-4'
          }
        >
          <Dialog.Panel>
            <div className="bg-slate-700 p-4 rounded text-white">
